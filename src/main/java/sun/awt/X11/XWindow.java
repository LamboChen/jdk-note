...
//  }
    ...
    */
    ...
    int keyEventType2Id( int xEventType ) {
        return xEventType == XConstants.KeyPress ? KeyEvent.KEY_PRESSED :
               xEventType == XConstants.KeyRelease ? KeyEvent.KEY_RELEASED : 0;
    }
    ...
    final void handleKeyPress(XKeyEvent ev) {
        ...
        if( jkc == null ) {
            jkc = new XKeysym.Keysym2JavaKeycode(KeyEvent.VK_UNDEFINED, KeyEvent.KEY_LOCATION_UNKNOWN);
        }
        ...
        int jkeyExtended = jkc.getJavaKeycode() == KeyEvent.VK_UNDEFINED ?
                           primaryUnicode2JavaKeycode( unicodeFromPrimaryKeysym ) :
                             jkc.getJavaKeycode();
        postKeyEvent( KeyEvent.KEY_PRESSED,
                          ev.get_time(),
                          isDeadKey ? jkeyExtended : jkeyToReturn,
                          (unicodeKey == 0 ? KeyEvent.CHAR_UNDEFINED : unicodeKey),
                          jkc.getKeyLocation(),
        ...
        if (unicodeKey > 0 && !isDeadKey) {
                ...
                postKeyEvent( KeyEvent.KEY_TYPED,
                              ev.get_time(),
    ...
    private void handleKeyRelease(XKeyEvent ev) {
        ...
        int jkeyExtended = jkc.getJavaKeycode() == KeyEvent.VK_UNDEFINED ?
                           primaryUnicode2JavaKeycode( unicodeFromPrimaryKeysym ) :
                             jkc.getJavaKeycode();
        postKeyEvent(  KeyEvent.KEY_RELEASED,
                          ev.get_time(),
                          isDeadKey ? jkeyExtended : jkeyToReturn,
                          (unicodeKey == 0 ? KeyEvent.CHAR_UNDEFINED : unicodeKey),
                          jkc.getKeyLocation(),
