...
public class File implements Serializable, Comparable<File> {
     ...
     *                           the directory
     */
    // 返回一个字符串数组，用于表示由 当前 path 表示的目录中的文件和目录
    public String[] list() {
        SecurityManager security = System.getSecurityManager();
        if (security != null) {
            security.checkRead(path);
     ...
     */
    // 将 path 创建文件夹。当文件夹被成功创建返回 true，否则返回 false
     ...
     */
    // 以 path 创建文件夹，如果父文件不存在则创建父文件
    public boolean mkdirs() {
        // check 当前文件夹是否存在，存在则返回 false
        if (exists()) {
            return false;
        }
